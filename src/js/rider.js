...
function setupShowHideRider() {
  [...document.querySelectorAll(".shower")].forEach((element) => {
...
function setupRaceGraph(element) {
  ...
  Highcharts.chart(element.getAttribute("id"), {
    ...
    tooltip: {
      formatter: function () {
        return `<span>${this.point.x} etappe: ${Math.abs(this.point.y).toFixed(
          1,
        )} plass % av antall i klasse</span>`;
      },
    },
    plotOptions: {
      series: {
...
function chooseTooltip(data, graph) {
  ...
  return function () {
    ...
    return `<span>${pointData.x} ${pointData.race} <br/> ${
      pointData.class
    }: ${pointData.y.toFixed(1)} % av antall i klasse </span>`;
  };
}
