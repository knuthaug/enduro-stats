...
class Converter {
  ...
  className (name) {
    ...
    } else if (/Kvinner Åpen.*17+/.test(name) || /Menn Åpen.*17+|Menn trim/i.test(name) || /.*Elduro.*/i.test(name) || /M\s?17-18/.test(name) || /Kvinner U23/.test(name) || /.* Åpen klasse el-sykkel/i.test(name)) {
      return name
