...
async function allRidersRankings(riders, currentYear) {
  let i = 1
  riders.forEach(async (rider) => {
    ...
    const { year, avg, score } = userRanking(results, currentYear)
    importDb.addRankings(rider.id, year, avg, score, raceIds.pop().date)
  })
}
...
function userRanking(rows, currentYear) {
  const years = { }
  const scores = []
