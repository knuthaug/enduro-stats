...
async function allRidersRankings (riders, currentYear, currentRaceId, currentRaceDate) {
  riders.forEach(async (rider) => {
    ...
    const { year, avg, score } = await userRanking(results, currentYear, currentRaceDate, rider)
    ...
    if (rows[i].time_behind !== 'DNS' && rows[i].time_behind !== 'DNF') {
      ...
      if (diffDays(parse(currentRaceDate), parse(rows[i].date)) <= 1095) {
        if (rows[i].rank !== 1 || (rows[i].rank / rows[i].count) < 1) {
          ...
          const winnerTime = await db.winnerTimeOfRace(rows[i].race, rows[i].gender, rows[i].uid)
          const diffTime = totalTime - winnerTime

          if (diffTime >= 0) { // filter out classes with fewer stages
