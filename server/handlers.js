...
async function raceHandler (req) {
  ...
  const sortedClasses = Object.keys(results)
    .sort()
    .map((e) => {
      return { name: e, anchor: cleanAnchor(e) }
    })
...
async function seriesHandler (req) {
  ...
  const sortedClasses = results
    .map((r) => r.name)
    .sort()
    .map((r) => {
      return { name: r, anchor: cleanAnchor(r) }
    })
