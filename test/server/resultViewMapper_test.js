...
const data = JSON.parse(
  fs
    .readFileSync(path.join(__dirname, '../data/race-results-complete.json'))
    .toString()
)
...
const data3 = JSON.parse(
  fs
    .readFileSync(
      path.join(__dirname, '../data/race-results-complete-class-diff.json')
    )
    .toString()
)
...
tap.test('total winner in class is first in result array', (t) => {
  ...
  t.equal(men[1].acc_time_behind, '01:51.45', 'total time behind is included')
  t.end()
})
...
tap.test('For stages either DNF or DNS, stage_time is replaced', (t) => {
  ...
  t.equal(
    men[74].stage6_time,
    'DNS',
    'stage time of zero is DNS if status is DNS'
  )
  t.end()
})
...
tap.test('For classes with fewer stages, total is calculated for last', (t) => {
  ...
  t.equal(
    sport[0].acc_time,
    '27:44.0',
    'total time is included for fewer stages'
  )
  t.equal(
    sport[0].acc_time_behind,
    '00:00.0',
    'total time is included for fewer stages'
  )
  t.equal(
...
tap.test('Calculate percent_behind_leader in stage', (t) => {
  const sport = r3.Sport
  t.equal(
    sport[0].stage1_percent_behind_leader,
    '0',
    'percent behind for winner is 0'
  )
  t.equal(
    sport[1].stage1_percent_behind_leader,
    '7.5',
    'percent behind for winner is 2.4'
  )
  t.end()
})
...
tap.test('has data object for graph, per class', (t) => {
  ...
  t.equal(
    graphs['Menn-acc-times'][1].data[0][1].toFixed(1),
    '35.4',
    'acc time behind in first is equal to behind in first'
  )
  t.equal(
    graphs['Menn-acc-times'][1].data[1][1].toFixed(1),
    '40.5',
    'acc time behind in second is equal to behind in first + second'
  )
  t.end()
})
...
tap.test('each row has graph object', (t) => {
  ...
  t.equal(second[0].data[1][1], -5.14)
...
tap.test('each row has acc_times_behind array', (t) => {
  const men = r.Menn
  t.equal(
    men[0].acc_behind_leader.length,
    6,
    'on value per stage, all 0 for winner'
  )
  t.equal(
    men[0].acc_behind_leader[0],
    '0.0',
    'on value per stage, all 0 for winner'
  )

  t.equal(
    men[1].acc_behind_leader[0],
    '35.4',
    'acc behind for first stage is same as behind in stage'
  )
  t.equal(
    men[3].acc_behind_leader[0],
    '33.2',
    'acc behind for first stage is same as behind in stage'
  )
  t.equal(
    men[3].acc_behind_leader[1],
    '95.5',
    'acc behind for first stage is same as behind in stage'
  )
  t.end()
...
tap.test('each row has time_behind_infront array', (t) => {
  const men = r.Menn
  t.equal(
    men[0].acc_behind_infront.length,
    6,
    'one value per stage, all 0 for winner'
  )
  t.equal(
    men[0].acc_behind_infront[0],
    '0.0',
    'one value per stage, all 0 for winner'
  )

  t.equal(
    men[4].acc_behind_infront[0],
    '8.6',
    'acc behind for first stage is same as behind in stage'
  )
  t.equal(
    men[4].acc_behind_infront[1],
    '-13.6',
    'acc behind for first stage is same as behind in stage'
  )
  t.equal(
    men[4].acc_behind_infront[2],
    '32.7',
    'acc behind for first stage is same as behind in stage'
  )
  t.end()
