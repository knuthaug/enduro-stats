...
const data = deepFreeze(
  JSON.parse(
    fs
      .readFileSync(path.join(__dirname, '../data/racesForSeries.json'))
      .toString()
  )
)
const riderData = deepFreeze(
  JSON.parse(
    fs
      .readFileSync(path.join(__dirname, '../data/series_for_rider.json'))
      .toString()
  )
)
const riderDataNc = deepFreeze(
  JSON.parse(
    fs
      .readFileSync(path.join(__dirname, '../data/series_for_rider_nc.json'))
      .toString()
  )
)
...
tap.test('sorts by overall placements', (t) => {
  ...
  t.equal(
    r[0].entries[0].uid,
    '82cf4f5e7c6f14635da21560a8ba44d8',
    'entries have uid'
  )
  t.equal(
    r[0].entries[0].avgRank,
    0.8333333333333334,
    'entries have avg. rank'
  )
  ...
  t.same(
    ...
    [
      ...
      {
        name: 'Traktorland enduro',
        rank: 1,
        totalInClass: 4,
        points: 500,
        status: 'OK'
      },
      { name: 'DrammEnduro', rank: 0, points: 0, status: 'OK' },
      {
        name: 'Oslo enduro',
        rank: 1,
        totalInClass: 5,
...
tap.test('sorts by overall placements NC', (t) => {
  ...
  t.equal(
    rNc[0].entries[0].avgRank,
    0.8333333333333334,
    'entries have avg. rank'
  )
  t.equal(rNc[0].entries[0].totalPoints, 3000, 'entries have total points')
  t.equal(rNc[0].entries[0].seriesrank, 1, 'entries have seriesrank')
  t.equal(
    rNc[0].entries[0].maxTotalPoints,
    3000,
    'entries have maxtotal points'
  )
  t.equal(
    rNc[0].entries[0].races.length,
    6,
    'all races in row, also not participated'
  )
  t.same(
    rNc[0].entries[1].races,
    [
      {
        name: 'Telemark enduro',
        rank: 2,
      ...
      { name: 'Oslo enduro', rank: 0, points: 0, status: 'OK' },
      {
        name: 'Hakadal enduro',
        rank: 2,
        totalInClass: 4,
  ...
  t.equal(rNc[0].entries[1].totalPoints, 2520, 'entries have total points')
  t.equal(
    rNc[0].entries[1].maxTotalPoints,
    2520,
    'entries have maxtotal points'
  )
  t.end()
...
tap.test('maps to years', (t) => {
  ...
  t.equal(
    rRider[1].years[1].races[0].raceName,
    'Hakadal enduro',
    'Østafjells second'
  )
