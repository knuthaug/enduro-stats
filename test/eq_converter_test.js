...
tap.test('include club', async t => {
  ...
  t.equals(data.results[0].name, 'Espen Johnsen', 'Name if first is Espen Johnsen')
  t.equals(data.results[0].gender, 'M', 'Gender is M')
  t.equals(data.results[0].rank, 1, 'rank is first')
  t.equals(data.results[0].stage_time_ms, 397000, 'time should be correct in ms')
  t.equals(data.results[0].class, 'Menn senior', 'class is menn senior')
  t.equals(data.results[0].club, 'Bergen MTB Klubb', 'club is correct')
  t.equals(data.results[0].team, '', 'team is empty')
  t.end()
})
...
tap.test('handle results with time in different named column', async t => {
  ...
  t.equals(data.results[0].name, 'Zakarias Blom Johansen')
  t.equals(data.results[0].gender, 'M')
  t.equals(data.results[0].rank, 1)
