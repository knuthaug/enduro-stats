...
tap.test('parse file with one stage in it', async t => {
  ...
  t.equal(r[0].acc_time_ms, 757200, 'acc_time_ms for first stage is same as stage')
...
tap.test('parse file and format, other year', async t => {
  ...
  t.equal(r[0].time, '12:47.8', 'time matches')
