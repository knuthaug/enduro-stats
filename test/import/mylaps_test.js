...
tap.test('parse complete result file', async t => {
  ...
  t.equal(data.race.uid, '9c5dc4c9a0ef9b18dc61f514ae9d1eab', 'race uid matches')
})
...
tap.test('Object details for stages', async t => {
  ...
  t.equal(rider.rider_uid, '3a4e5a2c99e22aaf931e06ac5d9320e7', 'uid is correct')
...
tap.test('final status field is correct', async t => {
  ...
  t.equal(lastStage.results[lastStage.results.length - 1].final_status, 'DNS', 'DNS rider has final_status DNS')
  t.equal(lastStage.results[lastStage.results.length - 17].final_status, 'DNF', 'DSQ rider has final_status DSQ')
  t.equal(lastStage.results[21].final_status, 'DSQ', 'DSQ rider has final_status DSQ')
  t.equal(lastStage.results[21].status, 'DSQ', 'DSQ rider has final_status DSQ')
  t.end()
})
...
tap.test('support for NOOP stages', async t => {
  ...
  t.equal(persons.length, 0, 'person should not have a stage 4, as class only has three stages')
})
